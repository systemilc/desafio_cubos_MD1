...
function solucao(min, km) {
    let minNormal = 20
let kmNormal = 10
const minExcedido = min - minNormal
const kmExcedido = km - kmNormal

let custoMinNormal = 50
let custoKmNormal = 70
let custoMinExcedido = 30
let custoKmExcedido = 50

let total = 0
  

    if(min <= minNormal && km <= kmNormal){ 
        total = (min * custoMinNormal) + (km * custoKmNormal)
    }

    if(min > minNormal && km <= kmNormal){
        total = 
        (minExcedido * custoMinExcedido) + 
        ((minNormal * custoMinNormal) + (km * custoKmNormal)) 
    }

    if(min <= minNormal && km > kmNormal){
        total = 
        min * custoMinNormal + 
        kmExcedido * custoKmExcedido +
        kmNormal * custoKmNormal
    }

    if(min > minNormal && km > kmNormal){
        total =
        minNormal * custoMinNormal +
        kmNormal * custoKmNormal +
        minExcedido * custoMinExcedido +
        kmExcedido * custoKmExcedido
    }
